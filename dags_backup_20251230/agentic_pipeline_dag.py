...
def self_healing_pipeline():
    ...
    @task
    def batch_analyse_sentiment(healed_reviews: list[dict], model_info: dict):
        if not healed_reviews:
            return []
        logger.info(f'Analysing sentiment for {len(healed_reviews)} reviews')
        return _analyze_with_ollama(healed_reviews, model_info)

    @task
    def aggregate_result(results: list[list[dict]]):
        context = get_current_context()
        params = context['params']
        ...
        summary = {
            ...
            'average_configuration': avg_confidence,
            'results': results,
        }
    ...
    analyzed_results = batch_analyse_sentiment(healed_reviews, model_info)

    summary = aggregate_result(analyzed_results)
